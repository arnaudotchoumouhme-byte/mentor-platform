...
export class SqliteNativeCatalogResources implements NativeCatalogResources {
  ...
  isResolvable(link: ResourceLink): boolean {
    ...
    switch (link.kind) {
      ...
      // Exact stable source/version pair; availability does not imply clinical approval.
      case "SOURCE": return this.database.all("SELECT v.source_version_id FROM source_versions v JOIN sources s ON s.source_id=v.source_id WHERE s.source_id=? AND v.source_version_id=? AND s.status='READY' AND v.extraction_status='COMPLETED'", ...args).length === 1;
