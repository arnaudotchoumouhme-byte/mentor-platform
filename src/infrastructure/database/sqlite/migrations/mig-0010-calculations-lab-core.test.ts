...
describe("MIG-0010 Calculations Lab Core",()=>{
  it("retains MIG-0010 in a fresh bootstrap through the current version",()=>{const sqlite=new DatabaseSync(":memory:");const result=new FreshDatabaseBootstrap(db(sqlite)).run();expect(result.currentVersion).toBe(19);expect(sqlite.prepare("SELECT migration_id FROM schema_migrations ORDER BY to_version").all()).toHaveLength(19);expect(sqlite.prepare("SELECT checksum FROM schema_migrations WHERE migration_id='MIG-0010'").get()).toEqual({checksum:migrationChecksum(calculationsLabCoreMigration)});sqlite.close();});
  it("migrates synthetic v9 through current and preserves legacy, MCQ and Foundation",()=>{const sqlite=new DatabaseSync(":memory:");const executor=db(sqlite);new FreshDatabaseBootstrap(executor,v9).run();sqlite.exec("INSERT INTO subjects(name) VALUES('legacy'); INSERT INTO curriculum_versions VALUES('cv','program',1,'DRAFT','2027',NULL,'2026',NULL); INSERT INTO mcq_question_items(item_id,latest_version) VALUES('item',1);");const result=new FreshDatabaseBootstrap(executor).run();expect(result).toMatchObject({currentVersion: 19,appliedMigrationIds:["MIG-0010","MIG-0011", "MIG-0012", "MIG-0013", "MIG-0014", "MIG-0015", "MIG-0016", "MIG-0017", "MIG-0018", "MIG-0019"]});expect(sqlite.prepare("SELECT name FROM subjects").get()).toEqual({name:"legacy"});expect(sqlite.prepare("SELECT item_id FROM mcq_question_items").get()).toEqual({item_id:"item"});expect(sqlite.prepare("PRAGMA integrity_check").get()).toEqual({integrity_check:"ok"});sqlite.close();});
