...
describe("MIG-0006 MCQ Core", () => {
  ...
  it("bootstraps a clean synthetic database through version 6", () => { const sqlite = new DatabaseSync(":memory:"); const result = new FreshDatabaseBootstrap(executor(sqlite)).run(); expect(result.currentVersion).toBe(6); const names = sqlite.prepare("SELECT name FROM sqlite_schema WHERE type='table'").all().map((row) => String(row.name)); for (const name of MCQ_CORE_TABLE_NAMES) expect(names).toContain(name); sqlite.close(); });
  it("enforces immutable references, ordered snapshots and one answer", () => { const sqlite = new DatabaseSync(":memory:"); sqlite.exec("PRAGMA foreign_keys=ON"); new FreshDatabaseBootstrap(executor(sqlite)).run(); expect(() => sqlite.prepare("INSERT INTO mcq_question_versions(item_id,version,stem,choices_json,correct_choice_id,explanation,difficulty) VALUES('missing',1,'s','[]','a','e','FOUNDATION')").run()).toThrow(); sqlite.close(); });
});
