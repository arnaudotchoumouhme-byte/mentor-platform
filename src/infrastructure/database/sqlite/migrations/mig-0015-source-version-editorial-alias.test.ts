...
describe("MIG-0015 source version editorial alias", () => {
  it("migrates a synthetic v14 database additively and preserves data", () => {
    ...
    const v14 = new MigrationRegistry(coreMigrationRegistry.migrations.filter(migration => migration.id !== "MIG-0015"));
    ...
    expect(new FreshDatabaseBootstrap(database, coreMigrationRegistry).run()).toEqual({ currentVersion: 15, appliedMigrationIds: ["MIG-0015"] });
