...
describe("ControlledMigrationActivation", () => {
  let root: string;
  ...
  it("dry-runs and activates a fresh synthetic database only with exact authorization", async () => {
    ...
    expect(request.actions.map(({ kind, migrationId }) => [kind, migrationId])).toEqual([
      ["EXECUTE", "MIG-0001"], ["EXECUTE", "MIG-0002"], ["EXECUTE", "MIG-0003"], ["EXECUTE", "MIG-0004"], ["EXECUTE", "MIG-0005"], ["EXECUTE", "MIG-0006"], ["EXECUTE", "MIG-0007"],
    ]);
    ...
    expect(await service.execute(databasePath, request, authorization(request))).toMatchObject({
      status: "MIGRATION_ACTIVATED", fromVersion: 0, toVersion: 7, verificationStatus: "VERIFIED",
    });
  });
  ...
  it("blocks ahead, checksum mismatch and incompatible current schemas", async () => {
    for (const mode of ["ahead", "checksum", "schema"] as const) {
      ...
      if (mode === "ahead") sqlite.prepare("INSERT INTO schema_migrations VALUES(?,?,?,?,?,?,?,?,?)").run("MIG-0008",7,8,"Future","0".repeat(64),"2026-01-01",0,"executed",null);
  ...
  it("invalidates authorization when the registry plan changes", async () => {
    ...
    const extra = { id: "MIG-0008", fromVersion: 7, toVersion: 8, description: "Synthetic", checksumMaterial: ["v1"], up: () => undefined };
    const changed = new ControlledMigrationActivation(new MigrationRegistry([coreBaselineMigration, importJournalMigration, sourceModelMigration, ragIndexMigration, clinicalCoachMigration, mcqCoreMigration, foundationAcademyCoreMigration, extra]));
