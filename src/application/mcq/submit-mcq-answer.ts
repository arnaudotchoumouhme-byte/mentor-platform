...
export class SubmitMcqAnswer implements UseCase<SubmitMcqAnswerInput, McqSession> {
  ...
  async execute(input: SubmitMcqAnswerInput): Promise<McqSession> {
    try {
      ...
      if (isMockExamExpired(session, answeredAt)) throw new McqError("MCQ_SESSION_EXPIRED", "Le temps de l’examen est écoulé.", "Mock Exam answer rejected at or after its deadline.");
      ...
      const answer = { itemId: input.itemId, itemVersion: input.itemVersion, choiceId: input.choiceId, correct, durationMs: input.durationMs ?? null, errorClassification: correct ? null : "UNCLASSIFIED", answeredAt } as const;
