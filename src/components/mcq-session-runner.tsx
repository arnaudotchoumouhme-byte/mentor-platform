...
export function McqSessionRunner() {
  ...
  const [error, setError] = useState<Readonly<{ code: string; traceId: string | null }> | null>(null);
  const [busy, setBusy] = useState(false);

  useEffect(() => { void (async () => {
    const response = await clientFetch("/api/mcq/sessions", { cache: "no-store" });
    if (!response.ok) { const failure = await body<Failure>(response); setError({ code: failure.error?.code ?? `HTTP_${response.status}`, traceId: failure.error?.traceId ?? response.headers.get("x-trace-id") }); setBlueprints([]); return; }
    setBlueprints((await body<{ blueprints: readonly Blueprint[] }>(response)).blueprints);
  })().catch(() => { setError({ code: "NET_REQUEST_FAILED", traceId: null }); setBlueprints([]); }); }, []);

  async function start() {
    const blueprint = blueprints?.[0]; if (!blueprint) return;
    ...
    } catch { setError({ code: "NET_REQUEST_FAILED", traceId: null }); }
    finally { setBusy(false); }
  }
  async function fail(response: Response) { const failure: Failure = await body<Failure>(response).catch(() => ({})); setError({ code: failure.error?.code ?? `HTTP_${response.status}`, traceId: failure.error?.traceId ?? response.headers.get("x-trace-id") }); }
  async function answer(choiceId: string) {
    const current = session?.items[index]; if (!session || !current || current.answer) return;
    setBusy(true); try {
      ...
      if (!response.ok) return void await fail(response); setSession(await body<PlayableSession>(response));
    } catch { setError({ code: "NET_REQUEST_FAILED", traceId: null }); } finally { setBusy(false); }
  }
  async function complete() {
    if (!session) return; setBusy(true);
    try { const response = await clientFetch(`/api/mcq/sessions/${session.sessionId}/complete`, { method: "POST" }); if (!response.ok) return void await fail(response); const result = await body<{ score: PlayableSession["score"] }>(response); setSession({ ...session, status: "COMPLETED", score: result.score }); }
    catch { setError({ code: "NET_REQUEST_FAILED", traceId: null }); } finally { setBusy(false); }
  }

  if (blueprints === null) return <div className="card p-8" role="status">Chargement du corpus QCM…</div>;
  if (error) return <div className="card p-8" role="alert"><strong>Le QCM est indisponible.</strong><p className="mb-0 text-sm text-[var(--muted-foreground)]">Code : {error.code}{error.traceId ? ` · traceId : ${error.traceId}` : ""}</p></div>;
  ...
  return <div className="card p-6 md:p-8"><div className="mb-6 flex justify-between"><span className="badge">Question {index + 1}/{session.items.length}</span><span className="badge">{current.difficulty}</span></div><div className="progress mb-7"><span style={{ width: `${((index + 1) / session.items.length) * 100}%` }}/></div><h2 className="text-xl leading-8">{current.stem}</h2><div className="my-6 grid gap-3">{current.choices.map(choice => <button key={choice.id} disabled={busy || current.answer !== null} onClick={() => void answer(choice.id)} className={`rounded-xl border p-4 text-left font-bold ${current.answer?.choiceId === choice.id ? "border-[var(--primary)] bg-[var(--accent)]" : "border-[var(--border)] bg-white hover:bg-[var(--muted)]"}`}>{choice.text}</button>)}</div>{current.answer && <div className="mb-5 rounded-xl bg-[var(--accent)] p-4"><strong>{current.answer.correct ? "Bonne réponse" : "À revoir"}</strong><p className="mb-0 text-sm leading-6">{current.answer.explanation}</p></div>}<div className="flex justify-end">{current.answer && (index < session.items.length - 1 ? <button className="btn btn-primary" onClick={() => setIndex(index + 1)}>Question suivante</button> : <button className="btn btn-primary" disabled={busy} onClick={() => void complete()}>Terminer</button>)}</div></div>;
}
