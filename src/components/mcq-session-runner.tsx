...
type PlayableSession = Readonly<{ sessionId: string; sessionKind?: "STANDARD" | "MOCK_EXAM" | null; status: "IN_PROGRESS" | "COMPLETED"; items: readonly PlayableItem[]; score: null | Readonly<{ percentage: number; correct: number; total: number }>; remainingSeconds?: number | null; deadlineAt?: string | null }>;
...
export function McqSessionRunner({ sessionKind = "STANDARD" }: Readonly<{ sessionKind?: "STANDARD" | "MOCK_EXAM" }>) {
  ...
  function acceptSession(next: PlayableSession) {
    setSession(next);
  ...
  const loadCatalog = useCallback(async () => {
    ...
    try {
      const response = await clientFetch(catalogUrl, { cache: "no-store" });
      if (!response.ok) {
  ...
  useEffect(() => {
    if (!isMockExam || session?.status !== "IN_PROGRESS" || remainingSeconds === null || remainingSeconds <= 0 || error) return;
    const timer = window.setInterval(() => setRemainingSeconds(value => value === null ? null : Math.max(0, value - 1)), 1_000);
    return () => window.clearInterval(timer);
  }, [error, isMockExam, remainingSeconds, session?.status]);
  useEffect(() => {
    if (!isMockExam || session?.status !== "IN_PROGRESS" || remainingSeconds !== 0 || operationInFlight.current || error) return;
    const timer = window.setTimeout(async () => {
      operationInFlight.current = true; setReconciling(true);
      try {
        const response = await clientFetch(`/api/mcq/sessions/${session.sessionId}`, { cache: "no-store" });
        if (!response.ok) { await deterministicFailure(response); return; }
        const current = await body<PlayableSession>(response); acceptSession(current);
        if (current.status === "COMPLETED") setResumableSessionId(null);
      } catch { setError({ code: "NET_REQUEST_FAILED", traceId: null, action: "RETRY_RESUME", sessionId: session.sessionId }); }
      finally { operationInFlight.current = false; setReconciling(false); }
    }, 0);
    return () => window.clearTimeout(timer);
  }, [error, isMockExam, remainingSeconds, session]);
  ...
  async function resume(sessionId = resumableSessionId) {
    ...
    try {
      ...
      acceptSession(current); setIndex(restoredIndex(current));
      if (current.status === "COMPLETED") setResumableSessionId(null);
  ...
  async function reconcileCreation(sessionId: string) {
    ...
    try {
      const response = await readOwnedSession(sessionId);
      if (response.ok) { acceptSession(await body<PlayableSession>(response)); setIndex(0); return; }
      const detail = await failure(response);
  ...
  async function answer(choiceId: string) {
    ...
    try {
      ...
      acceptSession(await body<PlayableSession>(response));
  ...
  async function complete() {
    ...
    try {
      ...
      acceptSession({ ...session, status: "COMPLETED", score: result.score, remainingSeconds: 0 });
