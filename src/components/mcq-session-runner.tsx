...
type ErrorAction = "CATALOG_RETRY" | "SAFE_EXIT" | "RETRY_CREATION" | "RETRY_ANSWER" | "RETRY_COMPLETION" | "LOCKED";
...
export function McqSessionRunner() {
  ...
  if (!session) return <div className="card p-8"><h3 className="mt-0">Corpus QCM versionné</h3><p>{blueprints.reduce((sum, value) => sum + value.itemCount, 0)} question(s) publiée(s) disponible(s).</p><button className="btn btn-primary" disabled={busy} onClick={() => void start()}>Commencer</button></div>;
