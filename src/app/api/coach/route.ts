...
export function createCoachPost(identity: () => Promise<PilotIdentity>, load: () => Promise<Coach>) {
  return async (request: Request) => {
    ...
    try {
      await identity(); const clinicalCoach = await load();
...
export const POST = createCoachPost(async () => (await import("@/infrastructure/pilot/server-pilot")).requirePilotIdentity(), async () => (await import("@/infrastructure/coach/server-clinical-coach")).clinicalCoach);
