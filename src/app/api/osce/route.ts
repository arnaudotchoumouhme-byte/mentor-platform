...
const uuid=z.string().uuid();const body=z.discriminatedUnion("action",[
 ...
 z.object({action:z.literal("interact"),sessionId:uuid,roleId:uuid,text:z.string().trim().min(1).max(4000),disclosureId:uuid.nullish()}).strict(),
 z.object({action:z.literal("reveal"),sessionId:uuid,disclosureId:uuid}).strict(),
 z.object({action:z.literal("complete"),sessionId:uuid}).strict(),
]);
...
export function createOsceHandlers(load:()=>Promise<OsceApi>){return{GET:async(request:Request)=>{const traceId=resolveTraceId(request.headers.get("x-trace-id"));const url=new URL(request.url);const id=uuid.safeParse(url.searchParams.get("id"));if(!id.success)return invalid(traceId);const view=url.searchParams.get("view")??"state";const api=await load();if(view==="station")return respond(traceId,()=>api.getStation(id.data));if(view==="replay")return respond(traceId,()=>api.replay(id.data));return respond(traceId,()=>api.state(id.data));},POST:async(request:Request)=>{const traceId=resolveTraceId(request.headers.get("x-trace-id"));let raw:unknown;try{raw=await request.json();}catch{return invalid(traceId);}const parsed=body.safeParse(raw);if(!parsed.success)return invalid(traceId);const api=await load();const x=parsed.data;if(x.action==="start")return respond(traceId,()=>api.start({...x,traceId}),201);if(x.action==="interact")return respond(traceId,()=>api.interact({...x,traceId}),201);if(x.action==="reveal")return respond(traceId,()=>api.reveal({...x,traceId}),201);return respond(traceId,()=>api.complete({...x,traceId}),201);}};}
