...
import type { PilotRepository } from "@/application/pilot/pilot-core";
...
describe("GET /api/canadian-practice", () => {
  it("returns Ontario provenance, version, verification and trace ID", async () => {
    const response = await createCanadianPracticeGet(async () => service() as unknown as CanadianPracticeQueries, identity)(request(`resource=active&practiceRuleId=${ruleId}&jurisdiction=PROVINCIAL&province=ON&at=2026-08-14T00%3A00%3A00.000Z`));
  ...
  it("supports federal, precise-version and history reads", async () => {
    const api = service(); const get = createCanadianPracticeGet(async () => api as unknown as CanadianPracticeQueries, identity);
  ...
  it("accepts Quebec while preserving provenance and jurisdiction metadata", async () => {
    ...
    const response = await createCanadianPracticeGet(async () => api as unknown as CanadianPracticeQueries, identity)(request(`resource=active&practiceRuleId=${ruleId}&jurisdiction=PROVINCIAL&province=QC&at=2026-08-14T00%3A00%3A00.000Z`));
  ...
  it("fails closed for invalid and unsupported provinces before loading infrastructure", async () => {
    const load = vi.fn(); const get = createCanadianPracticeGet(load, identity);
  ...
  it("maps not-effective and missing-version errors without exposing internals", async () => {
    ...
    const response = await createCanadianPracticeGet(async () => api as unknown as CanadianPracticeQueries, identity)(request(`resource=active&practiceRuleId=${ruleId}&jurisdiction=PROVINCIAL&province=ON&at=2026-08-14T00%3A00%3A00.000Z`));
    expect(response.status).toBe(422); expect(JSON.stringify(await response.json())).not.toContain("secret");
  });
});


describe("Canadian Practice pilot authorization", () => {
  it.each(["unauthenticated", "absent", "disabled"] as const)("denies %s before loading business data", async kind => {
    const repository = {findAccountBySubject: vi.fn(async () => kind === "disabled" ? {accountId: "a", learnerId: "l", oidcSubject: "auth0|test", status: "DISABLED", createdAt: "now", updatedAt: "now"} : null)} as unknown as PilotRepository;
    const access = new PilotAccessService(repository);
    const load = vi.fn();
    const get = createCanadianPracticeGet(load, () => access.resolve(kind === "unauthenticated" ? null : "auth0|test"));
    expect((await get(request(`resource=rule&practiceRuleId=${ruleId}`))).status).toBe(kind === "unauthenticated" ? 401 : 403);
    expect(load).not.toHaveBeenCalled();
  });
  it("awaits identity before any business load", async () => {
    const calls: string[] = [];
    const get = createCanadianPracticeGet(async () => { calls.push("load"); return service() as unknown as CanadianPracticeQueries; }, async () => { calls.push("identity"); return identity(); });
    expect((await get(request(`resource=rule&practiceRuleId=${ruleId}`))).status).toBe(200);
    expect(calls).toEqual(["identity", "load"]);
  });
});
