...
export function createSessionsGet(load:()=>Promise<{execute():Promise<unknown>}>,identity:()=>Promise<PilotIdentity>){return async(request:Request)=>{const traceId=resolveTraceId(request.headers.get("x-trace-id"));try{await identity();return NextResponse.json({blueprints:await(await load()).execute()},{headers:{"x-trace-id":traceId,"cache-control":"no-store"}});}catch(error){return apiErrorResponse(error,{traceId,module:"mcq",operation:"mcq.catalog.list"});}};}
...
export const GET=createSessionsGet(async()=>(await import("@/infrastructure/mcq/server-mcq")).mcqServices.list,async()=>(await import("@/infrastructure/pilot/server-pilot")).requirePilotIdentity());
