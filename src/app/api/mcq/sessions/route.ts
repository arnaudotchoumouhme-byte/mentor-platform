...
export function createSessionsGet(load:()=>Promise<{execute():Promise<unknown>}>,identity:()=>Promise<PilotIdentity>,findResumable:(learnerId:string)=>Promise<string|null>=async()=>null){return async(request:Request)=>{const traceId=resolveTraceId(request.headers.get("x-trace-id"));try{const caller=await identity();const [blueprints,resumableSessionId]=await Promise.all([(await load()).execute(),findResumable(caller.learnerId)]);return NextResponse.json({blueprints,resumableSession:resumableSessionId?{sessionId:resumableSessionId}:null},{headers:{"x-trace-id":traceId,"cache-control":"no-store"}});}catch(error){return apiErrorResponse(error,{traceId,module:"mcq",operation:"mcq.catalog.list"});}};}
...
export const GET=createSessionsGet(async()=>(await import("@/infrastructure/mcq/server-mcq")).mcqServices.list,async()=>(await import("@/infrastructure/pilot/server-pilot")).requirePilotIdentity(),async learnerId=>(await import("@/infrastructure/pilot/server-pilot")).pilotOwnership.findInProgressMcqSession(learnerId));
