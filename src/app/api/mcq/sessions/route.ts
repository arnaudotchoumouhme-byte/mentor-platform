...
const invalid=(traceId:string)=>NextResponse.json({success:false,error:{code:"VALIDATION_ERROR",message:"Requête MCQ invalide."}},{status:400,headers:{"x-trace-id":traceId}});

export function createSessionsPost(load:()=>Promise<UseCase<CreateMcqSessionInput,unknown>>,identity:()=>Promise<PilotIdentity>,bind:(sessionId:string,learnerId:string)=>Promise<void>){return async(request:Request)=>{const traceId=resolveTraceId(request.headers.get("x-trace-id"));let body:unknown;try{body=await request.json();}catch{return invalid(traceId);}const parsed=schema.safeParse(body);if(!parsed.success)return invalid(traceId);try{const caller=await identity();const result=await(await load()).execute({...parsed.data,traceId});await bind((result as {sessionId:string}).sessionId,caller.learnerId);return NextResponse.json(result,{status:201,headers:{"x-trace-id":traceId}});}catch(error){const response=mapErrorToHttp(error);return NextResponse.json(response.body,{status:response.status,headers:{"x-trace-id":traceId}});}};}
