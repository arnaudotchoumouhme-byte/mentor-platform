...
import { AI_PROVIDERS } from "@/presentation/settings/ai-providers";

export default function Settings() {
  const { data, act } = useAppState();
  return <><div className="mx-auto max-w-5xl"><AccountSecurity /></div>{data
    ? <SettingsForm key={JSON.stringify(data.settings)} initial={data.settings} act={act} />
    : <Loading />}</>;
}
...
export function SettingsForm({ initial, act }: { initial: Record<string, string>; act: (payload: object) => Promise<void> }) {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <form onSubmit={save} className="grid gap-6 lg:grid-cols-2">
      ...
      <section className="card p-6"><h2 className="mt-0">Intelligence artificielle</h2><label className="mb-4 block"><span className="label">Fournisseur IA</span><select className="field" value={form.aiProvider ?? "Mode local"} onChange={(event) => update("aiProvider", event.target.value)} disabled={busy}>{AI_PROVIDERS.map(provider => <option key={provider.id} value={provider.value} disabled={provider.status !== "available"}>{provider.label}{provider.status === "not-integrated" ? " (à configurer — connecteur non intégré)" : ""}</option>)}</select></label><label className="block"><span className="label">Budget quotidien maximal (CAD)</span><input type="number" min="0" step="0.25" className="field" value={form.dailyBudget ?? "2.00"} onChange={(event) => update("dailyBudget", event.target.value)} disabled={busy} /></label><div className="mt-4"><Notice>Seul le moteur local est disponible. Les fournisseurs externes nécessitent un connecteur serveur avant activation ; une clé seule ne suffit pas. Les secrets restent exclusivement côté serveur.</Notice></div></section>
