...
if(!data)return <Loading/>;async function search(e:React.FormEvent){e.preventDefault();const response=await clientFetch("/api/search",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({query,subject})});const body=await response.json();setResults(body.results??[]);setSearched(true)}
